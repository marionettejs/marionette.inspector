...
+ **Visualize** the view hiearchy with the UI tree
+ **Visualize** application activity with a full history of actions
+ **Inspect** view state: ui, events, listeners, properties
+ **Inspect** model state: attributes, listeners, properties
+ **Explore** Radio channel events, requests, commands
+ **Explore** application view's with an inspector magnifying glass
+ **Jump** between the inspector elements and source with intellgent links
